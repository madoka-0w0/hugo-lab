...
- 論文（`data/publications.yaml`）：架空の著者名・タイトル・雑誌・DOI を使用する。DOI プレフィックスは `10.0000/`（未割り当て）、タイトルには `[架空データ]` を末尾に付ける。実験名は "SAMPLE-EXP"・検出器名は "SAMPLE-DET"
